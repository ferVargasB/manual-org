...
{  
    ...
    try {
        if(empty($id_proceso_dependencia) || empty($id_actor))
        {
            throw new Exception('Existe un error, volver a intentarlo');
        }
        //guardar todos los campos en la bd
        $stmn = $objetoPDO->prepare("INSERT INTO procesos_actores_dependencias(id_proceso_dependencia,id_actor) VALUES(:id_proceso_dependencia,:id_actor)");
