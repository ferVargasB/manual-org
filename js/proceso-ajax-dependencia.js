$(document).ready(function () {
    ...
    const enableDepenciBtn = document.getElementById("habilitar_dependencia");
    const areaSelect = document.getElementById("area_perteneciente");
    const dependenciaSelect = document.getElementById("dependencia_perteneciente");

    enableDepenciBtn.addEventListener("change", e => {
        if (e.target.checked) {
            areaSelect.removeAttribute("required");
            areaSelect.setAttribute("disabled", "true");
            dependenciaSelect.removeAttribute("disabled");
            dependenciaSelect.setAttribute("required", "true");
        } else {
            dependenciaSelect.removeAttribute("required");
            dependenciaSelect.setAttribute("disabled", "true");
...
function add_actor() {
    if (this.hasAttribute("borrar")) {
        ...
        $.ajax({
            url: "servicios/ws_agregar_actores_procesos_dependencias.php?no_service=1",
            type: "POST",
...
function borrar_actor(boton) {
    ...
    $.ajax({
        url: "servicios/ws_eliminar_actores_procesos_dependencias.php?no_service=1",
        type: "POST",
        data: { id_proceso: $("#id_proceso").val(), id_actor: $(current_actor).val() },
        dataType: "JSON",
        success: function (data) {
