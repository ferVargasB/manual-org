...
async function main() {
    try {
        ...
        const res_actor_process = await fetch("servicios/ws_obtener_actores_procesos.php?numero_servicio=2", {
            method: "POST",
            body: form
        });
...
function set_actors(actors_per_process, all_actors) {
    ...
    if (document.getElementById("numero_actores").value !== actors_per_process.length) {
        //Este for renderiza los actores que faltan en un proceso
        total_actors = document.getElementById("numero_actores").value - actors_per_process.length;
...
async function addActor(actor, proceso, botonAdd) {
    try {
        ...
        const response = await fetch("servicios/ws_agregar_actores_procesos_dependencias.php?no_service=1", {
            method: "POST",
            body: data
        });
...
async function borrarActor(actor, proceso, botonBorrar) {
    try {
        ...
        data.append("id_proceso", proceso.value);
        data.append("id_actor", actor.value)
        const response = await fetch("servicios/ws_eliminar_actores_procesos_dependencias.php?no_service=1", {
            method: "POST",
            body: data
        });
        const resultado = await response.json();
